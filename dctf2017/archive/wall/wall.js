...
function player_can(player, direction) {
    return new Promise(function (resolve, reject) {
        ...
        switch (direction) {
            case 1:
                moving = !p.levels[l][y][x].left;
                break;
            case 2:
                moving = !p.levels[l][y][x].top;
                break;
            case 3:
                moving = !p.levels[l][y][x].right;
                break;
            case 4:
                moving = !p.levels[l][y][x].bottom;
                break;
            default:
...
function get_player_pos(player) {
    return new Promise(function (resolve, reject) {
        resolve(_p(players[player.id].status))
    });
}
...
function find_a_good_spot(player, status) {
    return new Promise(function (resolve, reject) {
        ...
        for (var d = 0; d < 4; d++) {
            ...
            player_can(player, d).then(cb.bind({
                    d: x[d]
                }))
...
function do_action(player, data) {
    if (data.type == 'move' && data.direction) {
        ...
        player_can(player, data.direction).then(function (status) {
            ...
            player.emit('status', JSON.stringify({
                pos: players[player.id].levels[status.level][status.pos.y][status.pos.x],
                level: status.level
            }));
        }).catch(function (e) {
